...
//    int Imb =  sum(Labels, P);
    ...
    int newx = P;
    double yval = 0;
//    double lmbdp = 0.5; // Note that the previous case corresponds
//    double lmbdm = 0.5; // to lmbdp=lmbdm=0.5 and attrp=attrm=0.
//    double attrp = 1;
//    double attrm = 1;
...
//    FILE *fp = NULL;
//    fp = fopen("/Users/yuru/Documents/GitHub/Monsyn/N1000P8000trial4.txt",  "r");
//    if( fp == NULL ) {
...
//    printf("Memory usage: %ld kilobytes\n",r_usage.ru_maxrss);
 ...
 #pragma omp parallel num_threads(6)
        ...
        for (int j  = 0; j < P ;j++ ){
            int k, t;
            k = rand() % P;
            t = Ordr[k]; Ordr[k] = Ordr[j]; Ordr[j] = t;
            IOrdr[Ordr[j]] = j;
...
//     printf("generate OrdrMat took %f seconds \n", cpu_time_used);
    ...
    for (int i = 0; i< N; i++){
            for (int k = 0; k < P; k++) {
                ActvMat[i][k] = (double) 0; //IOrdr[j] = j;
            }
        }
...
//    char *filepath = "/Users/yuru/Documents/GitHub/Monsyn/";
    ...
    fprintf(fp, "%d,%d,%d,%d,%d,%f",N, P, trial_id, NumError, j,cpu_time_used);
    fclose(fp);
